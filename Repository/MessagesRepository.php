...
class MessagesRepository extends Db {
  ...
  {
    $sql = "SELECT text, pseudo, DATE_FORMAT(date_message, '%W %e %M %Y') as date from message INNER JOIN user ON message.ID_user = user.ID WHERE adressee_id = :adresse_id AND ID_user = :id_user OR adressee_id = :id_user AND ID_user = :adresse_id ORDER BY message.ID ASC";
    $query = $this->connect()->prepare($sql);
    $query->execute([
      "adresse_id" => $adresse_id,
      "id_user" => $id_user
