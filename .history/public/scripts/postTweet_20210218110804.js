document.addEventListener("DOMContentLoaded", function (event) {
  ...
  document.querySelector(".retweet").addEventListener("submit", (e) => {
    e.preventDefault();
    const inputTweet = document.getElementById("postreTweet").value;
    let form = document.querySelector(".retweet");

    fetch("index.php?action=saveRetweet", {
      method: "POST",
      body: (data = new FormData(form)),
    })
      .then((response) => response.text())
