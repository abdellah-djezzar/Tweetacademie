document.addEventListener("DOMContentLoaded", function (event) {
  ...
  function likeValue(e) {
    let form = document.querySelector(".likeValue").value;
    fetch("index.php?action=likeTweet&id_user=" + e.target.value, {
      method: "POST",
      body: (data = new FormData(form)),
    })
      .then((response) => response.text())
      .then((response) => {
        document.getElementById("likeValue").innerHTML = response;
      })
      .catch((error) => console.log(error));
  }
});
