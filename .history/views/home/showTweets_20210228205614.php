...
foreach($retweets as $retweet){ ?>
  ...
  <button type="submit" class="likeValue far fa-heart" name="id_tweet" value="<?= $retweet["IDTweet"]?>">defaut</button>
  <?php } else{ ?>
  <button type="submit" class="likeValue fas fa-heart" name="id_tweet" value="<?= $retweet["IDTweet"]?>">aimé</button>
  <?php } ?>
  </form>
...
foreach($tweets as $tweet){ ?>
  ...
  <p value="<?=$tweet["textTweet"]?>"><?= $tweet["textTweet"]?></p>
        ...
        <?php if($tweet["isLiked"] == 0) { ?>
            <button type="submit" class="likeValue far fa-heart" name="id_tweet" value="<?= $tweet["IDTweet"]?>">par defaut</button>
        <?php } else { ?>
          <button type="submit" class="likeValue fas fa-heart" name="id_tweet" value="<?= $tweet["IDTweet"]?>">aimé</button>
          <?php } ?>
      </form>
        </div>
<?php 
