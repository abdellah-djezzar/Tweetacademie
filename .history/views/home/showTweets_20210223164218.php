...
foreach($retweets as $retweet){ ?>
  ...
  <button type="button" class="likeValue" name="id_tweet" value="<?= $retweet[0]?>">Like</button>
  <input type="hidden"  name="id_user">
  </form>
...
foreach($tweets as $tweet){ ?>
  ...
  <span>Tweet posted by: <a href="<?="index.php?action=profil&id_user=". $tweet["ID"] .""?>"><?=$tweet["pseudo"]?></a></span>
        ...
        <form action="" method="POST" class="likeForm">
            <button type="button" class="likeValue" name="id_tweet" value="<?= $tweet[0]?>">Like</button>
            <input type="hidden"  name="id_user">
      </form>
        </div>
<?php 
