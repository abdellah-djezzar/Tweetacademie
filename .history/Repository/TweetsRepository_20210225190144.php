...
class TweetsRepository extends Db {
  ...
  {
    $sql = "SELECT tweet.ID_user as UserIDTweet, tweet.text as textTweet, tweet.date as dateTweet, user.pseudo as pseudo, aime.ID_user as isLiked FROM tweet INNER JOIN user ON tweet.ID_user = user.ID LEFT JOIN aime ON aime.ID_user = user.ID ORDER BY tweet.ID DESC LIMIT 20";
    $query = $this->connect()->prepare($sql);
    $query->execute();
