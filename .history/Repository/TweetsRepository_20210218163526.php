...
class TweetsRepository extends Db {
  ...
  {
    $sql = "INSERT INTO tweet (ID, text, date) VALUES (:ID_user, :tweet, NOW())"; 
  ...
  {
    $sql = "SELECT * FROM follow INNER JOIN user ON id_following = user.ID WHERE follow.ID_user = :id_user";
    $query = $this->connect()->prepare($sql);
    $query->execute([
      "id_user" => $id_user
    ]);
  ...
  {
    $sql = "SELECT * FROM follow INNER JOIN user ON follow.ID_user = user.ID WHERE user.ID_user != :id_user";
    $query = $this->connect()->prepare($sql);
    $query->execute([
      "id_user" => $id_user,
    ]);
