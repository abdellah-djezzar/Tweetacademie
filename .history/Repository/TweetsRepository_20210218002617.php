...
class TweetsRepository extends Db {
  ...
  {
    $sql = "SELECT * FROM tweet INNER JOIN user ON tweet.ID_user = user.ID_user INNER JOIN retweet ON tweet.ID_tweet = retweet.ID_tweet ORDER BY ID_tweet DESC LIMIT 20";
    $query = $this->connect()->prepare($sql);
    $query->execute();
