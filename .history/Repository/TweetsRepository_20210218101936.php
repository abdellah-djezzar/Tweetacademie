...
class TweetsRepository extends Db {
  ...
  public function showRetweet($id_user)
  {
    $sql = "SELECT retweet.ID_tweet, tweet.text, tweet.ID_user,  user.pseudo, tweet.date from tweet INNER join retweet ON tweet.ID_user = retweet.ID_user INNER JOIN user ON user.ID_user = tweet.ID_user WHERE user.ID_user = 1";
  }
