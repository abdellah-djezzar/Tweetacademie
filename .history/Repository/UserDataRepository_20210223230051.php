...
{ 
    ...
    public function getAllFromUserByEmail(string $email) :array
    {
        $sql = "SELECT * FROM user 
                WHERE email = :email";
    ...
    public function darkTheme($id_user){
        $sql = "UPDATE user SET theme = 1 WHERE ID = :id_user";
        $query = $this->connect()->prepare($sql);
        $query->execute([
            "id_user" => $id_user
        ]);
    ...
    public function checkTheme($id_user) {
        $sql = "SELECT theme FROM user WHERE ID = :id_user";
        $query = $this->connect()->prepare($sql);
        $query->execute([
            "id_user" => $id_user
        ]);
