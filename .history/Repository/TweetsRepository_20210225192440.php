...
class TweetsRepository extends Db {
  ...
  {
    $sql = "SELECT tweet.ID_user as UserIDTweet, tweet.text as textTweet, tweet.date as dateTweet, tweet.ID as IDTweet, user.pseudo as pseudo,  IF(aime.ID_user != NULL, 0, 1) as isLiked, 'test' as test FROM tweet INNER JOIN user ON tweet.ID_user = user.ID LEFT JOIN aime ON aime.ID_user = user.ID ORDER BY tweet.ID DESC LIMIT 20";
    $query = $this->connect()->prepare($sql);
    $query->execute();
    return $query->fetchAll();
  }
  ...
  {
    $sql = "SELECT retweet.ID_tweet as IDTweet, tweet.text as textTweet, tweet.ID_user as UserIDTweet, user.pseudo as pseudo, tweet.date as dateTweet, IF(aime.ID_user != NULL, 0, 1) as isLiked from tweet 
    INNER join retweet ON tweet.ID = retweet.ID_tweet 
    INNER JOIN user ON user.ID = tweet.ID_user LEFT JOIN aime ON aime.ID_tweet = user.ID
