...
class TweetsRepository extends Db {
  ...
  {
    $sql = "SELECT * FROM tweet INNER JOIN user ON user.ID = tweet.ID_user WHERE user.ID = :id_user AND user.delete = 0 ORDER BY tweet.ID DESC LIMIT 20";
    $query = $this->connect()->prepare($sql);
    $query->execute([
      "id_user" => $id_user
    ]);
  ...
  {
    $sql = "SELECT * from user INNER JOIN follow ON follow.id_following = user.ID  WHERE follow.ID_user = :id_user AND user.delete = 0";
    $query = $this->connect()->prepare($sql);
    $query->execute([
      "id_user" => $id_user
    ]);
  ...
  {
    $sql = "SELECT * FROM follow INNER JOIN user ON follow.ID_user = user.ID WHERE id_following = :id_following AND user.delete = 0";
    $query = $this->connect()->prepare($sql);
    $query->execute([
      "id_following" => $id_following,
    ]);
  ...
  public function getAllFromFollowing(string $id_user) : array
  {
    $sql = "SELECT * FROM follow 
            ...
            WHERE follow.ID_user = :id_user AND user.delete = 0";
  ...
  {
    $sql = "SELECT COUNT(id_following) as nbrFollowing FROM follow INNER JOIN user ON follow.ID_user = user.ID WHERE user.ID = :id_user AND user.delete = 0";
    $query = $this->connect()->prepare($sql);
    $query->execute([
      "id_user" => $id_user
    ]);
  ...
  {
    $sql = "SELECT COUNT(id_following) as nbrFollowers FROM follow INNER JOIN user ON follow.ID_user = user.ID WHERE id_following = :id_user AND user.delete = 0";
  ...
  {
    $sql = "SELECT * FROM user WHERE id = :id_user AND delete = 0";
    $query = $this->connect()->prepare($sql);
    $query->execute([
      "id_user" => $id_user
    ]);
