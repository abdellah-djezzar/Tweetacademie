...
class TweetsRepository extends Db {
  ...
  {
    $sql = "SELECT * FROM tweet INNER JOIN user ON user.ID = tweet.ID_user WHERE user.ID = :id_user AND user.deleted = 0 ORDER BY tweet.ID DESC LIMIT 20";
    $query = $this->connect()->prepare($sql);
    $query->execute([
      "id_user" => $id_user
    ]);
  ...
  {
    $sql = "deleted FROM follow WHERE id_following = :id_following AND ID_user = :id_user";
    $query = $this->connect()->prepare($sql);
    $query->execute([
      "id_following" => $id_following,
      "id_user" => $id_user
  ...
  {
    $sql = "SELECT * from user INNER JOIN follow ON follow.id_following = user.ID  WHERE follow.ID_user = :id_user AND user.deleted = 0";
    $query = $this->connect()->prepare($sql);
    $query->execute([
      "id_user" => $id_user
    ]);
  ...
  {
    $sql = "SELECT * FROM follow INNER JOIN user ON follow.ID_user = user.ID WHERE id_following = :id_following AND user.deleted = 0";
    $query = $this->connect()->prepare($sql);
    $query->execute([
      "id_following" => $id_following,
    ]);
  ...
  public function getAllFromFollowing(string $id_user) : array
  {
    $sql = "SELECT * FROM follow 
            ...
            WHERE follow.ID_user = :id_user AND user.deleted = 0";
  ...
  {
    $sql = "SELECT COUNT(id_following) as nbrFollowing FROM follow INNER JOIN user ON follow.ID_user = user.ID WHERE user.ID = :id_user AND user.deleted = 0";
    $query = $this->connect()->prepare($sql);
    $query->execute([
      "id_user" => $id_user
    ]);
  ...
  {
    $sql = "SELECT COUNT(id_following) as nbrFollowers FROM follow INNER JOIN user ON follow.ID_user = user.ID WHERE id_following = :id_user AND user.deleted = 0";
  ...
  {
    $sql = "SELECT retweet.ID_tweet as IDTweet, tweet.text as textTweet, tweet.ID_user as UserIDTweet, user.pseudo as pseudo, tweet.date as dateTweet, IF(aime.ID_user IS NULL, 0, 1) as isLiked from tweet 
    INNER JOIN retweet ON tweet.ID = retweet.ID_tweet 
    INNER JOIN user ON user.ID = tweet.ID_user 
    LEFT JOIN aime ON tweet.ID = aime.ID
    WHERE user.deleted = 0
  ...
  {
    $sql = "SELECT * FROM user WHERE id = :id_user AND deleted = 0";
    $query = $this->connect()->prepare($sql);
    $query->execute([
      "id_user" => $id_user
    ]);
  ...
  {
    $sql = "SELECT * FROM tweet WHERE text LIKE :hashtag";
    $query = $this->connect()->prepare($sql);
    $query->execute([
      "hashtag" => "%#".$hashtag . '%'
    ]);
