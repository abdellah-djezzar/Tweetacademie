...
class TweetsRepository extends Db {
  ...
  {
    $sql = "SELECT tweet.ID_user as UserIDTweet, tweet.text as textTweet, tweet.date as dateTweet, tweet.ID as IDTweet, user.pseudo as pseudo,  IF(aime.ID_user != NULL, 0, 1) as isLiked FROM tweet INNER JOIN user ON tweet.ID_user = user.ID LEFT JOIN aime ON aime.ID_user = user.ID ORDER BY tweet.ID DESC LIMIT 20";
    $query = $this->connect()->prepare($sql);
    $query->execute();
    return $query->fetchAll();
  }
