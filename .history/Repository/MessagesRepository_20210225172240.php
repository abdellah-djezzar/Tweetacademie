...
class MessagesRepository extends Db {
  ...
  {
    $sql = "SELECT text, pseudo from message INNER JOIN user ON message.ID_user = user.ID WHERE adressee_id = :adresse_id AND ID_user = :id_user OR adresse_id = :ID_user AND ID_user = :adresse_id";
    $query = $this->connect()->prepare($sql);
    $query->execute([
      "adresse_id" => $adresse_id,
      "id_user" => $id_user
