...
class TweetsRepository extends Db {
  ...
  public function showTMyTweets($id_user) : array
  {
    $sql = "SELECT * FROM tweet INNER JOIN user ON tweet.ID_user = user.ID ORDER BY tweet.ID DESC LIMIT 20 WHERE user.ID = :id_user";
    $query = $this->connect()->prepare($sql);
    $query->execute([
      "id_user" => $id_user
    ]);
